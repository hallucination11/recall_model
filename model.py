...
class DSSM_sample_softmax(Model):

    def get_model_fn(self):

        def model_fn(features, labels, mode, params):

            # 此模型为dssm使用softmax loss，需要数据集全为正样本，在一个batch交叉做点积，同时需要每个item的概率用于修正（可作为一个feature使用）
            user_feature_embeddings = []
            # pos item embedding
            item_embeddings = []

            # 数据集中没有统计item出现概率 （None，1）,故用bal代替
            Q = tf.math.log(tf.cast(features['bal'] + 1, tf.float32))
            logQ = tf.reshape(tf.math.log(Q), (1, -1))

            for feature in ['uid', 'item', 'gender']:
                feature_emb = tf.compat.v1.feature_column.input_layer(features, params['feature_columns'][feature])
                if feature != 'item':
            ...
            user_net = tf.concat(user_feature_embeddings, axis=1, name='user')
            item_net = tf.concat(item_embeddings, axis=1, name='item')
            ...
            user_net /= params['temperature']
            print(user_net)
            print(item_net)
            # 注意这里是矩阵相乘，生成一个B * B 的矩阵, 同样label也要变成这种尺寸
            logits = tf.matmul(user_net, item_net, transpose_b=True)
            logits -= logQ
            label = tf.linalg.diag(tf.ones_like(logits[0]))

            if mode == tf.estimator.ModeKeys.PREDICT:
                predictions = {"user_emb": user_net,
                               "item_emb": item_net}
                return tf.estimator.EstimatorSpec(mode, predictions=predictions)
            print(label)
            print(logits)
...
class DSSM_simple_attentive_fusion(Model):
    ...
    def get_model_fn(self):
        ...
        def model_fn(features, labels, mode, params):
            ...
            W_u = tf.compat.v1.get_variable(name="embeddings", dtype=tf.float32, shape=[3 * 8, 3])

            a = tf.nn.softmax(tf.matmul(user_net, W_u))

            feature_embeddings_expend_dim = []
            for _ in feature_embeddings:
                _ = tf.expand_dims(_, axis=1)
                feature_embeddings_expend_dim.append(_)

            a = tf.reshape(a, shape=[-1, 3, 1])
            final_rep = tf.multiply(a, tf.concat(feature_embeddings_expend_dim, axis=1))
            print(tf.reduce_sum(final_rep, axis=1))
            exit()
            item_net = tf.concat(item_embeddings, axis=1, name='item')
    ...
    def get_estimator(self):
        ...
        cnt_feature_columns = {
            "uid": get_categorical_hash_bucket_column("uid", hash_bucket_size=2000, dimension=8, dtype=tf.int64),
            "item": get_categorical_hash_bucket_column("item", hash_bucket_size=100, dimension=8, dtype=tf.int64),
            "bal": get_bucketized_column("bal", boundaries=[10002.0, 14158.35, 18489.0, 23177.0, 27839.8, 32521.5,
                                                            36666.7, 41386.9, 45919.6, 50264.55, 54345.0], dimension=8),
            "gender": get_categorical_hash_bucket_column("gender", hash_bucket_size=2, dimension=8, dtype=tf.int64)
        }
